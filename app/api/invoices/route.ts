...
export async function GET() {
  ...
  const result = await query<InvoiceRow>(
    `SELECT id, prop_firm, program, amount::text AS amount, currency, invoice_date::text AS date, file_name, note
     FROM invoices
     WHERE user_id = $1
...
export async function POST(request: NextRequest) {
  ...
  const result = await query<InvoiceRow>(
    `INSERT INTO invoices
       (id, user_id, prop_firm, program, amount, currency, invoice_date, file_name, note)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
     RETURNING id, prop_firm, program, amount::text AS amount, currency, invoice_date::text AS date, file_name, note`,
    [
      crypto.randomUUID(),
      user.id,
      propFirm,
